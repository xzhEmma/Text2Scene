...
class DrawModel(nn.Module):
    ...
    def inference(self, input_inds, input_lens, start_step, explore_rate, explore_mode, ref_inds,x, adj):
        """
        Inputs: 
        ...
        # Text encoder input
        ##############################################################
        encoder_states = self.text_encoder(input_inds, input_lens, x, adj)
       # graph_f = self.gcn(x, adj)
        ##############################################################
        # Main Loop
        ##############################################################
        ...
        curr_shared_states = {'bg_imgs': bg_imgs}
        for i in range(self.cfg.max_output_length + 1): # add one EOS token
            ...
            curr_shared_states, curr_where_states = \
                self.scheduled_sample_step(env, curr_shared_states, encoder_states, 
                    curr_explore_rate, explore_mode, curr_inds)
            
            obj_logits_list.append(curr_shared_states['obj_logits'])
            coord_logits_list.append(curr_where_states['coord_logits'])
            attri_logits_list.append(curr_where_states['attri_logits'])
            if self.cfg.what_attn:
                what_attn_list.append(curr_shared_states['attn_wei'])
            if self.cfg.where_attn > 0:
                where_attn_list.append(curr_where_states['attn_wei'])
    ...
    def scheduled_sample_step(self, env, prev_states, encoder_states, \
        ...
        # Decode what
        ##############################################################
        prev_bg_imgs = prev_states['bg_imgs']
        bgfs = self.image_encoder(prev_bg_imgs)
        prev_states['bgfs'] = bgfs

        what_outs = self.what_decoder(prev_states, encoder_states)
        ...
        what_outs['fgfs'] = fgfs.unsqueeze(1)

        ##############################################################
        # Decode where
        ##############################################################
        where_outs = self.where_decoder(what_outs, encoder_states)
        expl_inds  = self.decode_where(where_outs, explore_mode)
        ...
        sample_inds_np = sample_inds.cpu().data.numpy()
        next_bg_imgs = env.batch_render_to_pytorch(sample_inds_np).unsqueeze(1)
        next_maps_np = env.batch_location_maps(sample_inds_np[:, 3])
        next_maps = torch.from_numpy(next_maps_np).unsqueeze(1).float()
        # Formatted as pytorch tensor/variable
        if self.cfg.cuda:
            next_bg_imgs = next_bg_imgs.cuda()
            next_maps = next_maps.cuda()

        what_outs['bg_imgs'] = next_bg_imgs
        what_outs['maps'] = next_maps

        return what_outs, where_outs
    ...
    def decode_where(self, where_states, sample_mode):
        ...
        if sample_mode == 0:
            _, sample_pose_inds = torch.max(pose_logits + 1.0, dim=-1, keepdim=True)
            _, sample_expr_inds = torch.max(expr_logits + 1.0, dim=-1, keepdim=True)
            _, sample_scal_inds = torch.max(scal_logits + 1.0, dim=-1, keepdim=True)
            _, sample_flip_inds = torch.max(flip_logits + 1.0, dim=-1, keepdim=True)
        else:
        ...
        sample_inds = torch.cat(
            [sample_pose_inds, sample_expr_inds, sample_coord_inds, sample_scal_inds, sample_flip_inds], 
            -1
        )

        return sample_inds

    def forward(self, input_inds, input_lens, start_step, explore_rate, explore_mode, ref_inds,x,adj):
        return self.inference(input_inds, input_lens, start_step, explore_rate, explore_mode, ref_inds,x,adj)

    def teacher_forcing(self, input_inds, input_lens, input_bg_imgs, input_fg_onehots, input_hmaps,x,adj):
        ...
        prev_states = {}
        prev_states['bgfs'] = bgfs
        prev_states['fgfs'] = input_fg_onehots[:,:-1]
        prev_states['maps'] = input_hmaps

        what_states  = self.what_decoder(prev_states, encoder_states)
        what_states['fgfs'] = input_fg_onehots[:,1:]
        where_states = self.where_decoder(what_states, encoder_states)
    ...
    def collect_logits(self, inf_outs, sample_inds):
        ...
        obj_logits   = inf_outs['obj_logits']
        coord_logits = inf_outs['coord_logits']
        attri_logits = inf_outs['attri_logits']

        # The first three indices are obj, pose, expr for historical reasons
        obj_inds   = sample_inds[:, :, 0].unsqueeze(-1)
        pose_inds  = sample_inds[:, :, 1].unsqueeze(-1)
        expr_inds  = sample_inds[:, :, 2].unsqueeze(-1)

        coord_inds = sample_inds[:, :, 3].unsqueeze(-1)
        scal_inds  = sample_inds[:, :, 4].unsqueeze(-1)
        flip_inds  = sample_inds[:, :, 5].unsqueeze(-1)
        ...
        sample_pose_logits  = torch.gather(pose_logits,  -1, pose_inds)
        sample_expr_logits  = torch.gather(expr_logits,  -1, expr_inds)
        sample_scal_logits  = torch.gather(scal_logits,  -1, scal_inds)
        # print('flip_inds', flip_inds)
        sample_flip_logits  = torch.gather(flip_logits,  -1, flip_inds)

        sample_logits = torch.cat(
            [ sample_obj_logits, sample_pose_logits, sample_expr_logits, 
              sample_coord_logits,  sample_scal_logits, sample_flip_logits ], 
            -1
        ).contiguous()
    ...
    def collect_accuracies(self, inf_outs, sample_inds):
        ...
        pose_inds  = sample_inds[:, :, 1]
        expr_inds  = sample_inds[:, :, 2]

        coord_inds = sample_inds[:, :, 3]
        scal_inds  = sample_inds[:, :, 4]
        flip_inds  = sample_inds[:, :, 5]
        ...
        _, pred_pose_inds  = torch.max(pose_logits,  -1)
        _, pred_expr_inds  = torch.max(expr_logits,  -1)
        _, pred_scal_inds  = torch.max(scal_logits,  -1)
        _, pred_flip_inds  = torch.max(flip_logits,  -1)


        obj_accu   = torch.eq(pred_obj_inds,   obj_inds  ).float()
        coord_accu = torch.eq(pred_coord_inds, coord_inds).float()
        pose_accu  = torch.eq(pred_pose_inds,  pose_inds ).float()
        expr_accu  = torch.eq(pred_expr_inds,  expr_inds ).float()
        scal_accu  = torch.eq(pred_scal_inds,  scal_inds ).float()
        flip_accu  = torch.eq(pred_flip_inds,  flip_inds ).float()

        sample_accus = torch.stack(
            [obj_accu, pose_accu, expr_accu, coord_accu, scal_accu, flip_accu], 
            -1
        )
    ...
    def topk_inference(self, input_inds, input_lens, K, start_step = 0, ref_inds=None):
        ...
        for i in range(self.cfg.max_output_length + 1): # add one EOS token
            ...
            if i < start_step:
                curr_shared_states, curr_where_states = \
                    self.scheduled_sample_step(env, curr_shared_states, encoder_states, -0.1, 0, curr_inds)
            else:
                accum_logprobs, accum_seq_msks, \
                curr_shared_states, encoder_states = self.decode_topk_step(
                    accum_logprobs, accum_seq_msks, 
                    curr_shared_states, encoder_states, env, K)
    ...
    def decode_topk_step(self, accum_logprobs, accum_seq_msks, prev_states, encoder_states, env, K):
        ...
        ##############################################################
        what_outs['rnn_outs'] = torch.index_select(what_outs['rnn_outs'], 0, beam_inds_1)
        what_outs['hids']     = torch.index_select(what_outs['hids'], 1, beam_inds_1)
        what_outs['bgfs']     = torch.index_select(what_outs['bgfs'], 0, beam_inds_1)
        if self.cfg.what_attn:
            what_outs['attn_ctx'] = torch.index_select(what_outs['attn_ctx'], 0, beam_inds_1)
            what_outs['attn_wei'] = torch.index_select(what_outs['attn_wei'], 0, beam_inds_1)

        encoder_states['rfts'] = torch.index_select(encoder_states['rfts'], 0, beam_inds_1)
        encoder_states['embs'] = torch.index_select(encoder_states['embs'], 0, beam_inds_1)
        encoder_states['msks'] = torch.index_select(encoder_states['msks'], 0, beam_inds_1)
        ...
        ##############################################################
        where_outs = self.where_decoder(what_outs, encoder_states)
        expl_inds  = self.decode_where(where_outs, 0)
        where_inds = expl_inds.clone()
        sample_inds = torch.cat([obj_inds, where_inds], 1)
        ...
        return accum_logprobs, accum_seq_msks, what_outs, encoder_states

    def decode_topk_where(self, raw_probs, accum_logprobs, accum_seq_msks, K, N=None):
        #####################################################################
        # Top N probs for each sample
        #####################################################################
        curr_probs = raw_probs.squeeze(1)
