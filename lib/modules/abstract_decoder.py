...
from modules.abstract_encoder import GCN
...
class WhatDecoder(nn.Module):
    def __init__(self, config):
        ...
        if self.cfg.what_attn:
            ...
            if self.cfg.use_fg_to_pred == 2:
                in_dim += fgf_dim #512+61=573
            # if self.cfg.sg:
            #     out_dim +=sg_dim #512+300+1024=1836
        ...
        if self.cfg.use_bg_to_pred:
            input_dim += bgf_dim
        if self.cfg.use_fg_to_pred == 2:
            input_dim += fgf_dim #512+512+300+61
        # if self.cfg.sg:
        #     input_dim +=sg_dim #
    ...
    def forward(self, prev_states, encoder_states):
       ...
       # print('prev_feats ', prev_feats.size())
        ...
        # print('att_rnns ', att_rnns.size())#[4,10,512]
        ...
        #################################################################
        if self.cfg.what_attn:
            encoder_feats = encoder_states['rfts'] #[4, 18, 512]
            if self.cfg.attn_emb:
...
class WhereDecoder(nn.Module):
    ...
    def forward(self, what_states, encoder_states):
        ...
        curr_fgfs = what_states['fgfs']#[4,10,61]
        ...
        if self.cfg.what_attn:
            ...
            if self.cfg.where_attn == 0:
                ...
                if self.cfg.sg:
                    encoder_feats = torch.cat([encoder_feats, encoder_states['sg']], -1)
        ...
        if self.cfg.sg:
            combined = torch.cat([combined, encoder_states['sg']], dim=2)
            print('combined 2', combined.size())
        #################################################################
        # Classifer
        #################################################################
