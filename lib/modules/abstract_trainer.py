...
class SupervisedTrainer(object):
    ...
    def batch_data(self, entry):
        ...
        return input_inds, input_lens, bg_imgs, fg_onehots, fg_hmaps, gt_inds, gt_msks, gt_scene_inds, x, adj
    ...
    def train_epoch(self, train_db, optimizer, epoch):
        ...
        for cnt, batched in enumerate(train_loader):
            ...
            gt_inds, gt_msks, gt_scene_inds, x,adj = \
            ...
            if self.cfg.teacher_forcing:
                inf_outs = self.net.teacher_forcing(input_inds, input_lens, bg_imgs, fg_onehots, fg_hmaps,x,adj)
            else:
    ...
    def validate_epoch(self, val_db):
        ...
        for G in range(2):
            ...
            for cnt, batched in enumerate(val_loader):
                ...
                self.net.eval()
                with torch.no_grad():
                    _, env = self.net(input_inds, input_lens, -1, 2.0, 0, None,x,adj)
                    ...
                    inf_outs, _ = self.net(input_inds, input_lens, -1, -0.1, 0, gt_inds,x,adj)
    ...
    def sample(self, epoch, test_db, N, random_or_not=False):
        ...
        for i in indices:
            ...
            self.net.eval()
            with torch.no_grad():
                inf_outs, env = self.net(input_inds, input_lens, -1, 2.0, 0, None,x,adj)
