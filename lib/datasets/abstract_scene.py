...
class abstract_scene(Dataset):
    def __init__(self, config, split=None, transform=None):
        ...
        self.graph_dir = osp.join(config.data_dir, 'graphs_txt1')
    ...
    def __getitem__(self, idx):
       ...
       # print('111',scene['scene_sentences'])
        ...
        graph_objs_p = []
        strr=' '
        for i in range (len(group_sents[cid])): 
            graph_obj_t=[]    
            graph = sng_parser.parse(group_sents[cid][i])
            #print(group_sents[cid][i])
            graph_objs_ls = graph['entities']
            graph_rels_ls = graph['relations']
            # print('graph_objs_ls',graph_objs_ls)
            # print('graph_rels_ls',graph_rels_ls)
            graph_rels_lss.append(graph_rels_ls)#i-th sentences k-th relations
            #print(graph_rels_lss)
            for j in range (len(graph_objs_ls)):         
                graph_obj = graph_objs_ls[j]['head']   
                #print(graph_obj)
                graph_obj = [w for w in word_tokenize(graph_obj.lower())] 
                #print(graph_obj)
                graph_obj = further_token_process(graph_obj)
                #print(graph_obj)
                if graph_obj==[] :
                    pass
                elif graph_obj[0] in self.lang_vocab.index2word:
                    graph_obj_t=graph_obj_t+[strr.join(graph_obj)]
                    #print('graph_obj_t',graph_obj_t)
                    while graph_obj not in graph_objs:
                        graph_objs.append(graph_obj)          
                else:  
                    graph_objs.append(['<pad>']) 
                    #print('graph_objs',graph_objs) #[['jenny'], ['crown'], ['tree'], ['mustard'], ['pie']]      
            graph_obj_tt.append(graph_obj_t)#[['jenny', 'crown'], ['jenny', 'tree'], ['mustard', 'pie']] 
            #print('graph_objs',graph_objs)
            #print(graph_obj_tt)
        str=' '
        obj_N = 18
        for objs in graph_objs:
            if len(objs)>1:
                graph_objs_p.append(str.join(objs))
            else:
                graph_objs_p.append(objs[0])
        adj_arr = np.zeros((obj_N,obj_N))
        #print(graph_objs_p)
        if len(graph_objs_p)<obj_N:
            graph_objs_p = graph_objs_p+['<pad>']*(obj_N-len(graph_objs_p))
        else:
            graph_objs_p = graph_objs_p[0:obj_N]
            #print('graph_objs_p',graph_objs_p)
                 
        rows = graph_objs_p
        columns = graph_objs_p
        adj = pd.DataFrame(adj_arr,index=rows,columns=columns)
        #print(graph_obj_tt)#[['mike', 'fire'], ['sun', 'cloud'], ['dog', 'cat', 'tree']]
        #[[{'subject': 0, 'object': 1, 'relation': 'warms', 'lemma_relation': 'warm'}, {'subject': 0, 'object': 2, 'relation': 'with', 'lemma_relation': 'with'}], [{'subject': 0, 'object': 1, 'relation': 'behind', 'lemma_relation': 'behind'}], []]
        for i in range (len(group_sents[cid])): 
            for k in range (len(graph_rels_lss[i])):
                
                sub = int(graph_rels_lss[i][k]['subject'])#0
                obj= int(graph_rels_lss[i][k]['object'])
                if sub in range(len(graph_obj_tt[i])):
                    if obj in range(len(graph_obj_tt[i])):
                        s = graph_obj_tt[i][sub]#0,0 mike
                        o = graph_obj_tt[i][obj] #fire
                        adj.loc[s,o]=1.0
                    else:
                        pass
                 
        #print(adj)
        entry['graph_adj'] = adj.values   
            
            # adj = torch.FloatTensor([[0.0]]).repeat(10, 10)
            # for k in range (len(graph_rels_ls)):
            #     sub = int(graph_rels_ls[k]['subject'])
            #     # print('sub',sub)
            #     # s = graph_objs_ls[sub]['span']
            #     # print('s',s)
            #     ss = graph_objs.index([s.lower()])
                 
               
            #     obj = int(graph_rels_ls[k]['object'])
            #     adj[sub][obj]=1.0 
            
        D = self.cfg.input_vocab_size
            # adj = torch.FloatTensor([[0.0]]).repeat(D, D) 
            # for k in range (len(graph_rels_ls)):
            #     sub = graph_obj_t[int(graph_rels_ls[k]['subject'])]   
            #     sub_ind = [self.lang_vocab.word2index[w] for w in sub]       
            #     obj = graph_obj_t[int(graph_rels_ls[k]['object'])]
            #     obj_ind = [self.lang_vocab.word2index[w] for w in obj]
            #     for s in sub_ind:
            #         for o in obj_ind:
            #             adj[s][o]=1.0 
        #entry['graph'] = adj   
        ...
        obj_inds = []
        #print('graph_objs',graph_objs)
        for i in range(len(graph_objs)):
            for w in graph_objs[i]:
                if w in self.lang_vocab.word2index:
                    obj_ind=[self.lang_vocab.word2index[w]]
                else:
                    obj_ind = [0]
            if obj_ind not in obj_inds:
                obj_inds.append(obj_ind)    #[[3], [336], [5, 268], [568], [122], [142]]

        entry['graph_objects'] = obj_inds
        #print(entry['graph_objects'])
  
###############################################################################
        num_obj = len(entry['graph_objects'])
        #print(entry['graph_objects'])
        objs = torch.LongTensor(obj_N).fill_(-1)
        objs_vector = torch.FloatTensor([[0.0]]).repeat(obj_N, D) 
        for i in range(num_obj):
            graph_rels_ls = graph['relations']
            for j in obj_inds[i]:
                #print('obj_inds[i]',obj_inds[i])
            # type in vector
                objs_vector[i][j] = 1.0  
        entry['objects_vector'] = objs_vector
        #print(objs_vector[0][3])
        ...
        # graphs_objs = []
        # for k in range(10020): #遍历文件夹     
        #     with open(self.graph_dir+"/"+"graph_%05d.txt"%(k), 'rb') as f:
        #         #print(self.graph_dir+"/"+"graph_%05d.txt"%(k))
        #         graph = f.read()
        #         graph = eval(graph)
        #         graph_objs_ls = graph['entities']
        #         graph_objs = []
        ...
        #     cur_objs = [w for w in word_tokenize(objs.lower())]
        #     print(cur_objs)
